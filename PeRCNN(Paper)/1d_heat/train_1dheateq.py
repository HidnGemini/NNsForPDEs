...
class upscaler(nn.Module):
    ...
    def __init__(self):
        ...
        self.layers = []
        self.layers.append(
            nn.ConvTranspose1d(1, 8, kernel_size=5, padding=5 // 2, stride=1, output_padding=1, bias=True))
        self.layers.append(torch.nn.Sigmoid())
        self.layers.append(
            nn.ConvTranspose1d(8, 8, kernel_size=5, padding=5 // 2, stride=1, output_padding=1, bias=True))
        self.layers.append(nn.Conv1d(8, 1, 1, 1, padding=0, bias=True))
...
class RCNNCell(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, input_kernel_size):
        ...
        self.W_laplace = nn.Conv1d(1, 1, self.input_kernel_size, self.input_stride, padding=0, bias=False)
        self.W_laplace.weight.data = 1/self.dx**2*torch.tensor(lap_1d_op, dtype=torch.float32)
        self.W_laplace.weight.requires_grad = False

        # Nonlinear term for u (up to 3rd order)
        self.Wh1_u = nn.Conv1d(in_channels=1, out_channels=hidden_channels, kernel_size=1,
                               stride=self.input_stride, padding=0, bias=True)
        self.Wh2_u = nn.Conv1d(in_channels=1, out_channels=hidden_channels, kernel_size=1,
                               stride=self.input_stride, padding=0, bias=True)
        self.Wh3_u = nn.Conv1d(in_channels=1, out_channels=hidden_channels, kernel_size=1,
                               stride=self.input_stride, padding=0, bias=True)
        self.Wh4_u = nn.Conv1d(in_channels=hidden_channels, out_channels=1, kernel_size=1,
...
class RCNN(nn.Module):
    ...
    def __init__(self, input_channels, hidden_channels, init_state_low, input_kernel_size,
        ...
        self._all_layers = []
        # self.init_state = torch.nn.Parameter(torch.tensor(ini_state, dtype=torch.float32).to(device), requires_grad=False)
        self.init_state_low = init_state_low.unsqueeze(0).unsqueeze(0)
        self.init_state = []

        # Upconv as initial state generator
        self.UpconvBlock = nn.ConvTranspose1d(in_channels=1, out_channels=1, kernel_size=4, stride=2, padding=1)
...
class loss_generator(nn.Module):
    ...
    def get_phy_Loss(self, output):  # Staggered version
        ...
        # spatial derivatives
        laplace_u = self.laplace(output[0, 0:-2, :])  # TODO: UPDATE THIS COMMENT (201x1x128x128)

        # temporal derivatives - u
        u = output[:, 0:1, 2:-2, 2:-2]
        lent = u.shape[0]
        lenx = u.shape[2]
        u_conv1d = u.permute(2, 1, 0)  # [width(X), c, step]
        u_conv1d = u_conv1d.reshape(lenx, 1, lent)
        u_t = self.dt(u_conv1d)  # lent-2 due to no-padding
        u_t = u_t.reshape(lenx, 1, lent-2)
        u_t = u_t.permute(2, 0, 1)  # [step-2, c, height(Y), width(X)]
...
def get_ic_loss(model):
    ...
    init_state_bicubic = F.interpolate(init_state_low, (100), mode='bicubic')
    mse_loss = nn.MSELoss()
...
def loss_gen(output, loss_func):
    # get physics loss
    mse_loss = nn.MSELoss()
    u = loss_func.get_phy_Loss(output)
    loss = mse_loss(u, torch.zeros_like(u).to(device))
    return loss
...
def pretrain_upscaler(Upconv, init_state_low, epoch=4000):
    ...
    :return:
    '''
    cs = sp.interpolate.CubicSpline([i for i in range(len(init_state_low))], init_state_low.cpu())
    cs(2.5)
    small_dx = 1/2*len(init_state_low)
    print(init_state_low)
    init_state_upscaled_list = [float(cs(x/small_dx)) for x in range(2*init_state_low.shape[0])]
    print(init_state_upscaled_list)
    init_state_upscaled = torch.Tensor(init_state_upscaled_list).to(device)
    ...
    for epoch in range(epoch):
        ...
        loss = mse_loss(init_state_pred, init_state_upscaled)
        loss.backward(retain_graph=True)
        print('[%d] loss: %.9f' % ((epoch+1), loss.item()))
        optimizer.step()
        scheduler.step()

def train(model, truth, n_iters, time_batch_size, learning_rate, dt, dx, cont):
    ...
    for epoch in range(n_iters):
        ...
        mse_loss = nn.MSELoss()
        # 21x2x50
        pred, gt = output[0:-1:20, :, ::4], truth[::20, :, ::4].to(device)
        ...
        if (epoch+1)%100 == 0:
            ...
            }, './PeRCNN(Paper)/2d_gs_rd/model/checkpoint.pt')
...
def add_noise(truth, pec=0.05):  # BxCx25
...
if __name__ == '__main__':
    ...
    data = sio.loadmat('./PeRCNN(Paper)/1d_heat/1x1001x25_heat_eq_data.mat')['tensor']
    UV = np.transpose(data, (1, 0, 2)).astype(np.float32)             # 2001x1x25
    truth_clean = torch.from_numpy(UV[:1001])  # [201, 2, 100, 100]
    # Add noise 10%
    ...
    truth = UV[:time_steps+1]                                                    # [201, 2, 25]
    ...
    U0_low = IC[0, 0, ::4]
    init_state_low = U0_low.to(device)
    print(init_state_low)
    ...
    fig_save_path = './PeRCNN(Paper)/1d_heat/figures/'
