...
class Upscaler(nn.Module):
    ...
    def __init__(self):
        ...
        self.net = nn.Sequential(
            nn.ConvTranspose1d(1, 8, kernel_size=9, padding=5 // 2, stride=3, output_padding=3, bias=True),
            nn.Sigmoid(),
            nn.ConvTranspose1d(8, 8, kernel_size=9, padding=5 // 2, stride=3, output_padding=3, bias=True),
...
def get_ic_loss(model):
    ...
    init_state_upscaled = F.interpolate(model.init_state_low, (56), mode='linear')
...
def pretrain_upscaler(Upconv, init_state_low, epochs=4000):
    init_state_upscaled = F.interpolate(init_state_low.unsqueeze(0).unsqueeze(0), (56), mode='linear')
